...
/*   Updated: 2023/11/19 14:59:23 by asabri           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calcu_yoffset(t_data *data, t_ray *ray)
{
	if (ray->wall_deriction == NORTH)
		ray->yoffset = ((ray->ystart - ((HEIGHT / 2.0) - \
				(ray->project_plan / 2.0))) \
...
void	calculate_distence(t_ray *ray, t_data *data, double ray_start)
{
	...
	{
		ray->player_ray_dist = ray->distanceh \
			* cos(data->player->rotationAngle - ray_start);
	...
	{
		ray->player_ray_dist = ray->distancev \
			* cos(data->player->rotationAngle - ray_start);
